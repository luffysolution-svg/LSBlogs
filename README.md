...
1. 直接读取和修改 `LSBlogs` 这份唯一的正式内容源。
2. 显示本次新增、修改和删除的提交文件清单。
3. 执行 `npm run build`。
4. 只暂存受管理的博客路径，不执行 `git add .`。
5. 使用当前仓库已有的 `origin` 和目标分支推送。

未发布草稿、待处理队列、本机绝对路径和图床密钥只保存在本机。管理器支持 Lsky Pro、腾讯云 COS、阿里云 OSS 和 GitHub 图床；私密配置写入被 Git 忽略的 `manager_data`，不会进入站点配置或前端构建。管理器不会生成 SSH 密钥，也不会把 Token 写入模板。
